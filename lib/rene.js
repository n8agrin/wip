(function() {
  ...
  rene.chart = function() {
    var chart, groupData, layerDelay, layerDuration, layers, legend, mapData, margin, origScales, positionData, resetXYScales, scales, xAxis, xAxisLabel, yAxis, yAxisLabel;
    margin = {
      top: 20,
      bottom: 20,
      left: 30,
    ...
    xAxisLabel = null;
    yAxisLabel = null;
    mapData = function(datasets) {
      var aesthetic, aestheticFns, aesthetics, dataset, datasetIdx, fn, layer, newDatasets, newPoint, newPoints, point, _i, _j, _len, _len2, _len3;
      aesthetics = Object.keys(scales);
      aesthetics.push('group');
      newDatasets = [];
      for (datasetIdx = 0, _len = datasets.length; datasetIdx < _len; datasetIdx++) {
        dataset = datasets[datasetIdx];
        layer = layers[datasetIdx];
        aestheticFns = {};
        for (_i = 0, _len2 = aesthetics.length; _i < _len2; _i++) {
          aesthetic = aesthetics[_i];
          if (layer[aesthetic]) aestheticFns[aesthetic] = layer[aesthetic]();
        }
        newPoints = [];
        for (_j = 0, _len3 = dataset.length; _j < _len3; _j++) {
          point = dataset[_j];
          newPoint = {};
          for (aesthetic in aestheticFns) {
            fn = aestheticFns[aesthetic];
    ...
    groupData = function(datasets) {
      var dataset, k, newDatasets, v, _i, _len, _ref;
      newDatasets = [];
      for (_i = 0, _len = datasets.length; _i < _len; _i++) {
        dataset = datasets[_i];
        if ((_ref = dataset[0]) != null ? _ref.group : void 0) {
          newDatasets.push((function() {
            var _ref2, _results;
    ...
    positionData = function(datasets) {
      var dataset, datasetIdx, newDatasets, _base, _len;
      newDatasets = [];
      for (datasetIdx = 0, _len = datasets.length; datasetIdx < _len; datasetIdx++) {
        dataset = datasets[datasetIdx];
        newDatasets.push(((typeof (_base = layers[datasetIdx]).position === "function" ? _base.position() : void 0) || Object)(dataset));
      }
      return newDatasets;
    };
    ...
    chart = function(selection) {
      return selection.each(function(datasets) {
        var aesValues, aesthetic, dataGroup, dataset, gEnter, idx, layer, layerGroups, panelHeight, panelWidth, point, positionedData, scale, scaleData, svg, xGridAxis, xLabel, xLabelEnter, yGridAxis, yLabel, yLabelEnter, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref, _ref2;
        ...
        if (scales.x) {
          ...
          if (xAxisLabel) {
            xLabel = d3.select('.x.axis').selectAll('text.label').data([xAxisLabel]);
        ...
        if (scales.y) {
          ...
          if (yAxisLabel) {
            yLabel = d3.select('.y.axis').selectAll('text.label').data([yAxisLabel]);
